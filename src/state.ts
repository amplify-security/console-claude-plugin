...
export function dataDir(env: Record<string, string | undefined> = process.env): string {
    return env.CLAUDE_PLUGIN_DATA ?? join(homedir(), ".claude", "plugins", "data", "console")
}
...
export function readCheckedShas(gitDir: string): Set<string> {
    const path = join(gitDir, CHECKED_FILE)
    if (!existsSync(path)) return new Set()
    const shas = readFileSync(path, "utf8")
        .split("\n")
 ...
 */
export function appendCheckedSha(gitDir: string, sha: string): void {
    appendFileSync(join(gitDir, CHECKED_FILE), `${sha}\t${new Date().toISOString()}\n`)
}
